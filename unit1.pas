...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure SeparatingSign(Sender: TObject);
    ...
    procedure MathSign(Sender: TObject);
  ...
  private
    ...
    SignPointed, SetToZero, Flag, Paint1, FlagArg: boolean;
    LastSign: char;//Last operation`s sign
  public
    { public declarations }
  end;
...
procedure TForm1.SeparatingSign(Sender: TObject);
begin
  WhereIsMySign(TButton(Sender).Caption);
end;
...
procedure TForm1.MathSign(Sender: TObject);
begin
  FlagArg := True;
  WhereIsMySign(TButton(Sender).Caption);
end;
...
begin
  ...
  begin
    ...
    begin  {ввод числа}
      ...
      if (not SignPointed) and (FlagArg) then
    ...
    begin
      case sign of
        '←': if (Text <> '0') and (LastSign <> '=') then
          begin
            check := copy(Text, Length(Text), Length(Text));
            if check = ',' then
              Paint1 := False;
